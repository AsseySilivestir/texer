...
// ════════════════════════════════════════════════════════════════════
//  HEALTH, TURN CREDENTIALS & OPTIONS
// ════════════════════════════════════════════════════════════════════

def handleHealth($req, $res) {
    dict $turnStatus = sua.webrtc.turn.status();
    $res.json({
        "status": "ok",
        "language": "Bantu",
        "framework": "Sua",
        ...
        "app": "ChatBantu",
        "version": "1.3.0",
        "turn": $turnStatus.json,
        "serverTime": nowMs()
    });
}

// ─── Embedded TURN credentials endpoint ─────────────────────────
// Frontend (call.html) fetches this to configure RTCPeerConnection.
// No external TURN server needed — Bantu provides it natively.
def handleTurnCredentials($req, $res) {
    // Pass empty string to let Bantu auto-detect the public IP.
    // In production, set the HOST env var to your server's public IP.
    dict $config = sua.webrtc.turn.config("");
    // The config.json is a raw JSON string — send it directly
    $res.set("Content-Type", "application/json");
    $res.send($config.json);
}

// ─── TURN status endpoint (debug/admin) ─────────────────────────
def handleTurnStatus($req, $res) {
    dict $status = sua.webrtc.turn.status();
    $res.set("Content-Type", "application/json");
...
// Embedded TURN (STUN/ relay) — auto-started, no external config
sua.server.get("/api/turn/credentials",               handleTurnCredentials);
sua.server.get("/api/turn/status",                     handleTurnStatus);
...
// ─── Auto-start embedded STUN/TURN relay ────────────────────────────
// One line. That's it. No Docker, no Coturn, no config files.
// Bantu developers never need to think about TURN again.
print "";
print "═══════════════════════════════════════════════════════════";
print "  Starting embedded STUN/TURN relay server...";
dict $turnResult = sua.webrtc.turn.start(3478);
if ($turnResult.ok) {
    print "  TURN relay: ON (port " + str($turnResult.port) + ")";
} else {
    print "  TURN relay: FAILED (calls will use STUN-only fallback)";
}
print "═══════════════════════════════════════════════════════════";
...
print "  TURN:     Embedded (port 3478) — no external server needed";
print "  Endpoints:";
...
print "    GET    /api/turn/credentials  (auto-generated ICE config)";
print "    GET    /api/turn/status";
print "═══════════════════════════════════════════";
print "";
